...
1. Primero, creo un volumen llamado "mi_volumen_ejercicio3" y arranco el contenedor con el siguiente comando:

   ```docker volume create mi_volumen_ejercicio3```

   ![captura 7](./imagenes/captura7.png)

2. Ejecuto el contenedor basado en "postgres:17" haciendo uso de `--detach` (en segundo plano) y añadiendo una contraseña con el flag `-e` (lo exige la imagen oficial de postgres).
...
3. Me conecto a la BBDD y creo la tabla, para entrar a la terminal de PostgreSQL (psql) dentro del contenedor que está corriendo uso `docker exec`:
...
5. Ahora detengo y destruyo el contenedor:

   ![captura 11](./imagenes/captura11.png)

6. Creo un contenedor completamente nuevo (le llamo "mi_nuevo_postgres"), pero le conecto el volumen "mi_volumen_ejercicio3" que guardó los datos del contenedor anterior:     

   ![captura 12](./imagenes/captura12.png)

7. Ahora voy a comprobar que los datos siguen existiendo, me conecto al nuevo contenedor usando "postgres" para verificar si la tabla y el registro sigue existiendo:
...
3. Instalo "ping" en el primer contenedor. Por defecto, las imágenes oficiales de Ubuntu en Docker vienen minimizadas para pesar lo mínimo posible, así que no traen la herramienta "ping". Voy a entrar al primer contenedor para instalarla:

   ![captura 18](./imagenes/captura18.png)

4. Ahora que "ubuntu1" ya tiene "ping", voy a pedirle que intente comunicarse con "ubuntu2" usando su nombre:

   ![captura 19](./imagenes/captura19.png)

#### Pregunta
¿Los contenedores pueden comunicarse entre sí?
Sí, se comunican perfectamente. Al ejecutar el comando "ping" "ubuntu2", se ve que "ubuntu1" recibe respuesta de inmediato. Al crear una red privada personalizada "my-net", Docker activa automáticamente un servidor DNS interno. Este servidor mapea el nombre de cada contenedor con su IP privada de forma dinámica
...
Explicación:
 - `volumes`: (al final del archivo): Declaro un volumen compartido llamado "mi_volumen_compartido". Docker se encargará de crearlo automáticamente en el sistema.
 - Servicio `writer`:
